// Async thumbnail generator with in-memory cache.
//
// Wraps `QLThumbnailGenerator` behind an `actor` so concurrent callers from
// multiple SwiftUI cells share a single cache without manual locking. The
// cache is keyed by file path + requested size + scale; entries are
// `NSImage` references suitable for direct rendering by `Image(nsImage:)`.
//
// Cache policy is intentionally minimal: a coarse capacity guard wipes the
// entire dictionary at `maxCacheCount`. This avoids LRU bookkeeping (and
// the synchronization cost it implies) while still bounding memory for
// shelves that hold hundreds of items. Errors from `QLThumbnailGenerator`
// are logged and converted to `nil` so cells can fall back to an SF Symbol
// placeholder.
...
public actor ThumbnailService {
    ...
    ///     Defaults to 96×96 to match the grid cell.
